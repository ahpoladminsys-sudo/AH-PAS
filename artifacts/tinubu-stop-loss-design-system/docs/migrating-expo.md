# Migrating Expo UI to Tinubu Stop Loss Design System

Read `artifacts/tinubu-stop-loss-design-system/docs/AGENTS.md` and
`artifacts/tinubu-stop-loss-design-system/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/tinubu-stop-loss-design-system/lib/native-theme` and
  `@workspace/tinubu-stop-loss-design-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/tinubu-stop-loss-design-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/tinubu-stop-loss-design-system` and retains only
