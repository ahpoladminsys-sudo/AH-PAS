    ...
    <div className="space-y-4">
      <section className="rounded-xl border bg-primary p-6 text-primary-foreground">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary-foreground/70">
          Tinubu visual language
        </p>
        <h2 className="mt-2 font-serif text-2xl font-semibold">
          Clear decisions, calm evidence, confident handoffs.
        </h2>
        <p className="mt-2 max-w-2xl text-sm text-primary-foreground/80">
          Use deep navy for trust and primary decisions, teal for active progress,
          mint for cleared states, gold for review, and red only for blocking risk.
        </p>
      </section>
      ...
      <section className="grid gap-5 rounded-xl border bg-card p-5 text-card-foreground lg:grid-cols-2">
        <div>
          <h2 className="font-semibold">Composition guidelines</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Extracted from the workspace’s underwriting and CRM surfaces.
          </p>
        </div>
        <Guidelines
          items={[
            { kind: 'do', text: 'Lead dense operational screens with a clear page title, status, and next action.' },
            { kind: 'do', text: 'Use cards and bordered sections to separate evidence, decisions, and supporting details.' },
            { kind: 'do', text: 'Use mono type for identifiers, rates, and audit values where comparison matters.' },
            { kind: 'dont', text: 'Do not use red for ordinary status or decoration; reserve it for blocked or destructive outcomes.' },
            { kind: 'dont', text: 'Do not hide source evidence behind an AI result; keep review and provenance visible.' },
          ]}
        />
      </section>
    </div>
